...
const Faqs = lazy(() => import('../../components/course/faqs'));
const Hero = lazy(() => import('../../components/course/hero'));
const CourseOverviewToolsAndModules = lazy(() => import('../../components/course/course_overview_tool_modules'));
const IsthisCourseRightForYou = lazy(() => import('../../components/course/course_overview_is_this_course_right_for_you'));
const UpcomingDemoSession = lazy(() => import('../../components/course/course_overview_upcoming_demo_session'));
const StickyNav = lazy(() => import('../../components/course/sticky_nav'));
const MobileStickyNav = lazy(() => import('../../components/course/mobile_sticky_nav'));
const Curriculum = lazy(() => import('../../components/course/curriculum'));
const InstructorDetails = lazy(() => import('../../components/course/instructor_details'));
const PricingPlans = lazy(() => import('../../components/course/learning_options_pricing_plans'));
const PlacementPathSteps = lazy(() => import('../../components/course/placement_path_steps'));
const ComapniesWaitngForYou = lazy(() => import('../../components/course/placement_path_companies_waiting_for_you'));
const TopCertifications = lazy(() => import('../../components/course/top_certifications'));
const CourseRegistrationForm = lazy(() => import('../../components/course/RegistrationForm'));
const SubscribeNewsLetters = lazy(() => import('../../components/course/SubscribeNewaLetters'));
const ElevateYourLearning = lazy(() => import('../../components/course/learning_options_elevate_your_learning'));
const RelatedBlogs = lazy(() => import('../../components/course/related_blogs'));
const RelatedCourses = lazy(() => import('../../components/course/related_courses'));
const AlreadySubmitted = lazy(() => import('../blog/details/already_submitted'));
const YoutubeVideoPopupPlayer = lazy(() => import('../../components/course/YoutubeVideoPopupPlayer'));
const WhatOurStudentsSay = lazy(() => import('../../components/course/curriculum_what_our_students_say'));
const JobTrends = lazy(() => import('../../components/course/job_trends'));
const Seo = lazy(() => import('../Seo'));
const SchemaLoader = lazy(() => import('../../components/course/SchemaLoader'));
const UpcomingDemoHeaderStrip = lazy(() => import('../../components/course/upcoming_demo_header_strip'));
const LearningOptionsDeserveUpgrade = lazy(() => import('../../components/course/learning_options_deserve_upgrade'));
const CourseOverviewKeyFeatures = lazy(() => import('../../components/course/course_overview_key_features'));
const CourseOverviewHandsonProjects = lazy(() => import('../../components/course/course_overview_handson_projects'));
const CourseOverviewWhyTechLeadSIT = lazy(() => import('../../components/course/course_overview_why_techleadsit'));
import httpService from '../../services/httpService';
import { useInView } from 'react-intersection-observer';
// import PlacementPathSuccessAlumni from '../../components/course/placement_path_success_alumni';
import Image from 'next/image';
const CoursePage = ({ slug, filePath, courseData, nativeCourse, changedData, courseId, courseTax }) => {
    ...
    const formConfigs = {
        ...
        "Download Course Brochure": {
            ...
            heading: "Download A Course Brochure",
            buttonLabel: "Submit",
        },
        "Watch Demo Video": {
            fields: ["fullName", "email", "phone"],
    ...
    // Memoize the function using useCallback for reusability
    const getRelatedBlogsAndCourses = useCallback(async (category) => {
        try {
            setLoading(true);
            const response = await httpService.get(
                `courses/getBlogsAndCoursesByCategory?categoryName=${encodeURIComponent(category)}`
            );
    ...
    useEffect(() => {
        if (courseData?.category) {
            getRelatedBlogsAndCourses(courseData.category);
        }
    }, [courseData?.category, getRelatedBlogsAndCourses]);

    const [demos, setDemos] = useState([])
    const [upcomingDemoDate, setUpcomingDemoDate] = useState("");
    ...
    useEffect(() => {
        const getUpcomingDemos = () => {
            const fetchDemos = async () => {
                setLoading(true);
                try {
                    const response = await httpService.post("courses/getUpcomingDemos", {
                    ...
                    if (response && response.data) {
                        ...
                        const demoDate = getNearestUpcomingDate(response.data.upcomingDemos);
                        setUpcomingDemoDate(demoDate);
                    }
                } catch (error) {
                    setLoading(false);
                    console.error('Error fetching demo sessions:', error?.response?.data?.message || error.message || error);
                }
            };

            fetchDemos();
        }
        //console.log("Course ID is: ", courseData?.id);
        getUpcomingDemos();
    }, [courseData?.id])
    ...
    return (<>
        ...
        <main className="Course-Page-CP">
            ...
            {showStrip && showStripByScroll && <div className="Main-Course-Banner-Strip" id="mainCourseStrip">
                ...
                <div className="d-flex fl-d-cl-aic">
                    <i className="fa-solid fa-tags Price-mb-cust" style={{ color: "rgb(0, 111, 170);" }}></i>
                    <div className="ms-2 mainCoursetext">
            ...
            <div className='white-space-issue'>
                <div className="Main-Course-Nav-Container">
                    ...
                    <MobileStickyNav showJobTrends={jobTrendsNotRequired.indexOf(courseData?.id)} ></MobileStickyNav>
                    <a href={typeof courseId === 'number' && courseId > 0 ? `/direct_payment/${courseId}` : '#'} target='_blank' rel="noopener noreferrer" className={`Main-Course-Top-Nav-CTA-Btn Main-Course-CTA-Button-Pop-Section ${!(typeof courseId === 'number' && courseId > 0) ? 'disabled' : ''}`}><span>Buy
                        Now</span>
                        <i className="fa-solid fa-arrow-right Main-Course-CTA-arrow Main-Course-CTA-Top-Nav-arrow ms-2"></i></a>
                </div>
                ...
                <section id="course-overview" ref={courseOverviewRef} className="Main-Course-T-scroll-container">
                    <section className="Main-Course-Top-Navbar">
                        ...
                        {loadBelowFold && <><IsthisCourseRightForYou data={courseData?.courseOverview?.isThisCourseRightForYou}></IsthisCourseRightForYou>
                            ...
                            <CourseOverviewHandsonProjects id={courseData?.id} openForm={openForm}></CourseOverviewHandsonProjects>
                            ...
                            <section className="Main-Course-CTA-banner2">
                ...
                {loadBelowFold && <><section id="curriculum" className="Main-Course-Top-Navbar">
                    ...
                    {courseData?.curriculum?.demoVideoPath && courseData?.curriculum?.demoVideoThumbnail && <section className="Main-Course-Watch-Our-Demo-Section">
                        ...
                        <YoutubeVideoPopupPlayer videoPath={courseData?.curriculum?.demoVideoPath} youtibeopenVideoPopup={youtibeopenVideoPopup} handleYoutibeOpenVideoPopup={handleYoutibeOpenVideoPopup}></YoutubeVideoPopupPlayer>
                    </section>}
                    ...
                    <section id="learning-options" className="Main-Course-Top-Navbar">
                        <PricingPlans data={courseData?.learningOptions?.pricingPlans} openForm={openForm} courseId={courseId} courseTax={courseTax}></PricingPlans>
            ...
            {loadBelowFold && <section className="Main-Course-Common-Sections">
                <section className="Main-Course-Success-Stories-Of-Our-Proud-Achievers-Section mt-3">
                    <h2 className="Main-Course-Success-Stories-Of-Our-Proud-Achievers-Heading text-center mb-4">
                ...
                <RelatedCourses courses={courses} courseTax={courseTax}></RelatedCourses>

                <Faqs data={courseData?.faqs} openForm={openForm}></Faqs>

                <RelatedBlogs blogs={blogs}></RelatedBlogs>
